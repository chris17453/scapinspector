...
def get(root,namespaces,file):
    benchmark_nodes=profile_core.nodes(root,"data_stream_collection/component/Benchmark",namespaces)
    benchmark_list=[]
    rules_list={}
    profile_count=0
    load_profiles=True
    ...
    for benchmark_node in benchmark_nodes:
        ...
        profile_nodes          = profile_core.nodes      (benchmark_node,"Profile"     ,namespaces)
        benchmark_title        = profile_core.node_values(benchmark_node,"title"       ,namespaces)
        benchmark_description  = profile_core.node_values(benchmark_node,"description" ,namespaces)
        benchmark_profiles     = []
        ...
        if load_profiles is True:
            for profile in profile_nodes:
                profile_count+=1
                profile_id           = profile_core.get_id_or_ref(profile,root,namespaces)
                profile_select_nodes = profile_core.nodes      (profile,"select"      ,namespaces)
                profile_title        = profile_core.node_values(profile,"title"       ,namespaces)
                profile_description  = profile_core.node_values(profile,"description" ,namespaces)
        ...
        benchmark_groups = group_get(benchmark_node,rules_list,namespaces)
...
def group_get(root,rules_list,namespaces):
    group_nodes = profile_core.nodes(root,"Group",namespaces)

    groups=[]
    for group_node in group_nodes:
        group_id              = group_node['id']
        group_selected        = group_node['selected']
        group_title           = profile_core.node_values(group_node,"title" ,namespaces)
        group_description     = profile_core.node_values(group_node,"description" ,namespaces)
        group                 = {}
        group['id']           = group_id
        ...
        local_rules=[]        
        group_rules=rules.get_at_this_level(group_node,namespaces)
        #print(group_rules)
        ...
        groupsresults=group_get(group_node,rules_list,namespaces)
